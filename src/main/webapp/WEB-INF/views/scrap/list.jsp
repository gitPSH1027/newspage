...
<script type="text/javascript">
	$(function () {
		...
		$('#delBt1').on('click',function(){
				...
				$('#frm').attr('action','${path }/scrap/del').attr('method','post').submit();
		});
		...
		$('.alist').on('click', function(e) {
			...
			location.href = '${path}/scrap/list?curpage='+curpage+'&findkey='+findkey+'&findvalue='+findvalue;
		});
});
</script>
</head>
<body>
	<div id="showScrapList">
		<button id="allSelect">전체선택</button>
		<button id="delBt1">삭제</button>
		<form id="frm" class="form-inline" action="">
			<select name="findkey">
				<option value="title"<c:out value="${page.findkey == 'title'? 'selected':'' }" />>제목</option>
				<option value="description"  <c:out value="${page.findkey == 'description'? 'selected':'' }" />>내용</option>
				<option value="titleAndDescription"<c:out value="${page.findkey == 'titleAndDescription'? 'selected':'' }" />>제목+내용</option>
			</select>
			<div class="form-group">
				<input type="text" name="findvalue" class="form-control" id="findvalue" value="${page.findvalue }"> <button id="selectBt1">조회</button>
			</div>
			<table class="table table-striped">
				...
				<tbody class="table-striped">
					<c:forEach var="i" items="${slist }">
						<tr>
							<td>
								<input type="checkbox" name="delSnum" value="${i.snum }">
								${i.snum }
							</td>
